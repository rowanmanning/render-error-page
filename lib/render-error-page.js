...
module.exports = function renderErrorPage(options) {
	...
	return (error, request, response, next) => { // eslint-disable-line no-unused-vars
		if (options.errorLoggingFilter(error)) {
			options.errorLogger(options.errorLoggingSerializer(error), request);
		}
		...
		response.render(options.errorView, renderContext, (renderError, html) => {
			if (renderError) {
				...
				options.errorLogger(options.errorLoggingSerializer(renderError), request);
 ...
 * @property {Function<Error, import('express').Request>} [errorLogger=console.error]
