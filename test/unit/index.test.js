		...
		describe('middleware(error, request, response, next)', () => {
			...
			describe('when `response.render` calls back with an error', () => {
				...
				it('responds with fallback HTML', () => {
					...
					td.verify(express.mockResponse.send(td.matchers.contains('Error 456')), {
						times: 1
					});
					td.verify(express.mockResponse.send(td.matchers.contains('mock error')), {
						times: 1
					});
					td.verify(
						express.mockResponse.send(
							td.matchers.contains('There was also an issue rendering the error page')
						),
						{ times: 1 }
					);
					td.verify(express.mockResponse.send(td.matchers.contains('<pre>')), {
						times: 1
					}); // Indicates that stacks are present
				});
			...
			describe('when `options.includeErrorStack` is `false`', () => {
				...
				describe('when `response.render` calls back with an error', () => {
					...
					it('responds with fallback HTML containing no error stacks', () => {
						...
						td.verify(express.mockResponse.send(td.matchers.contains('<pre>')), {
							times: 0
						}); // Indicates that stacks are not present
					});
				});
			});
			...
			describe('when `options.includeErrorStack` is not set', () => {
				...
				describe('when `process.env.NODE_ENV` is "production"', () => {
					...
					describe('when `response.render` calls back with an error', () => {
						...
						it('responds with fallback HTML containing no error stacks', () => {
							td.verify(express.mockResponse.send(td.matchers.isA(String)), {
								times: 1
							});
							td.verify(express.mockResponse.send(td.matchers.contains('<pre>')), {
								times: 0
